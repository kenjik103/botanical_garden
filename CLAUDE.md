...
- **Vanilla JS** only, for small visual flourishes.
- **Cloudflare Pages** for hosting; deploys on `git push`.

## Hard constraints — do NOT do these

- No audio player. (The "Winamp" reference is purely visual style.)
- No SPA / client-side page-swap layer (Swup, barba, View Transitions). Plain multi-page
  navigation with full page loads is intended.
...
Greenfield. Follow the milestone order in PLAN.md. **Start with milestone 1** (scaffold +
confirm `pelican --listen` works) and **do milestone 2 (deploy to Cloudflare Pages) early,
while the site is nearly empty** — do not defer deployment to the end.
